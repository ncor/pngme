...
const FILE_MINIMUM_LENGTH: usize = HEADER_LENGTH + PNG_CHUNK_MINIMUM_LENGTH;

type Header = [u8; HEADER_LENGTH];
...
pub enum PngParsingError {
    #[error("expected at least {FILE_MINIMUM_LENGTH} bytes, got {0}")]
    InvaidLength(usize),
    #[error("expected header {expected:?}, got {got:?}")]
    InvalidHeader { expected: Header, got: Vec<u8> },
    #[error("expected chunk data length of length 4 (u32), got (usize)")]
    InvalidChunkDataLengthLength(usize),
    #[error(transparent)]
    InvalidChunk(#[from] PngChunkParsingError),
}

#[derive(Error, Debug)]
#[error("could find chunk with type {chunk_type}")]
pub struct ChunkWithTypeNotFoundError {
    chunk_type: String,
}
...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(bytes: &[u8]) -> Result<Self, Self::Error> {
        if bytes.len() < FILE_MINIMUM_LENGTH {
            return Err(PngParsingError::InvaidLength(bytes.len()));
        }

        let header: Header = bytes[0..HEADER_LENGTH].try_into().unwrap();

        if header != Png::STANDARD_HEADER {
            return Err(PngParsingError::InvalidHeader {
                expected: Png::STANDARD_HEADER,
                got: header.to_vec(),
            });
        }
        ...
        let body_iter = bytes[HEADER_LENGTH..].iter();
        let mut last_chunk_end_index: usize = 0;

        loop {
            let chunk_bytes_iter = body_iter.clone().skip(last_chunk_end_index);

            if chunk_bytes_iter.clone().len() == 0 {
                break;
            }

            let maybe_chunk_data_length =
                chunk_bytes_iter.clone().take(PNG_CHUNK_DATA_LENGTH_LENGTH);
            if maybe_chunk_data_length.len() < PNG_CHUNK_DATA_LENGTH_LENGTH {
                return Err(PngParsingError::InvalidChunkDataLengthLength(
                    maybe_chunk_data_length.len(),
                ));
            }
            let chunk_data_length_bytes: [u8; 4] = maybe_chunk_data_length
                .cloned()
                .collect::<Vec<u8>>()
                .try_into()
                .unwrap();

            let chunk_data_length = u32::from_be_bytes(chunk_data_length_bytes);

            let required_chunk_length = PNG_CHUNK_DATA_LENGTH_LENGTH
                + PNG_CHUNK_TYPE_LENGTH
                + (chunk_data_length as usize)
                + CRC_LENGTH;

            let chunk_bytes = &chunk_bytes_iter
                .take(required_chunk_length)
                .cloned()
                .collect::<Vec<u8>>()[..];
            let chunk = PngChunk::try_from(chunk_bytes)?;

            chunks.push(chunk);

            last_chunk_end_index += required_chunk_length;
        }
...
impl Png {
    ...
    pub fn remove_first_chunk(
        ...
        if let Some(chunk_index) = maybe_chunk_index {
            ...
            Err(ChunkWithTypeNotFoundError {
...
mod tests {
    ...
    fn chunk_from_strings(chunk_type: &str, data: &str) -> Result<PngChunk, anyhow::Error> {
        use std::str::FromStr;

        let chunk_type = PngChunkType::from_str(chunk_type)?;
        let data: Vec<u8> = data.bytes().collect();

        Ok(PngChunk::new(chunk_type, data))
    }
